export const navigation = [
  {
    id: "0",
    title: "Home",
    url: "/home",
    img: "../icons/home.png"
  },
  {
    id: "1",
    title: "Explore",
    url: "/discover",
    img: "../icons/discover.png"
  },
  {
    id: "2",
    title: "Dashboard",
    url: "/dashboard",
    img: "../icons/wallet.png"
  },
  {
    id: "3",
    title: "Messages",
    url: "/messages",
    img: "../icons/message.png"
  },
  {
    id: "4",
    title: "Subscriptions",
    url: "/subscriptions",
    img: "../icons/subscriptions.png"
  },
  {
    id: "5",
    title: "Settings",
    url: "/settings",
    img: "../icons/setting.png"
  },
  
];
export const settings = [
  {
    id: "0",
    title: "Profile",
    url: "#profile-setting",
    
  },
  {
    id: "1",
    title: "Account",
    url: "#account-settings",
    
  },
  {
    id: "2",
    title: "Privacy & Safety",
    url: "#privacy",
    
  },
  {
    id: "3",
    title: "Notifications",
    url: "#notifications",
   
  },
  
  
];
let username = 'Abbot'
export const menuModals = [

  { id: "0", heading: 'Subscribe', pinkBtn: 'Subscribe ₦3,000.00/month', text: 'Support your favorite creators for bonus content and extra perks.', whiteBtn: 'Cancel' },
  {id: "2", heading: 'Hide comment', pinkBtn: 'Hide comment', text: `To give you more control over the conversations you start, post authors can hide replies to their posts.`, whiteBtn: 'Cancel' },
  {id: "3", heading: `Block ${username}`, pinkBtn: 'pinkBtn 3', text: `They won't have the ability to track your account or access your posts, and you won't receive posts or notifications from ${username}`, whiteBtn: 'modal3' , username: true},
 
];
export const menu = [
  {
    // id: "0",
    alt: "subscribe",
 text: 'Subscribe to ',
    img: "../icons/profile-tick.png"
  },
  {
    // id: "1",
    alt: "insight",
    text: 'View Insight',
    img: "../icons/chart.png"
  },
  {
    // id: "2",
    alt: "block",
    text: 'Block ',
    img: "../icons/slash.svg"
  },
  {
    // id: "3",
    alt: "Report",
    text: 'Report ',
    img: "../icons/flag.png"
  },
  
 
  
];
export const mobileNav = [
  {
    // id: "0",
    title: "Home",
    url: "/home",
    img: "../icons/home.png"
  },
  {
    // id: "1",
    title: "Explore",
    url: "/discover",
    img: "../icons/discover.png"
  },
  {
    // id: "2",
    title: "Notifications",
    url: "/notifications",
    img: "../icons/notification.png"
  },
  {
    // id: "3",
    title: "Messages",
    url: "/messages",
    img: "../icons/message.png"
  },
  
 
  
];

export const profileMenu = [
  {
    // id: "0",
    title: "Profile",
    url: "/profile",
    img: "../icons/profile.png"
  },
  {
    // id: "1",
    title: "Settings",
    url: "/settings",
    img: "../icons/setting.png"
  },
  {
    // id: "2",
    title: "bookmark",
    url: "/bookmark",
    img: "../icons/bookmark.png"
  },
  {
    // id: "3",
    title: "Subscriptions",
    url: "/subscriptions",
    img: "../icons/subscriptions.png"
  },
  
  // {
  //   // id: "3",
  //   title: "Log out",
  //   // logout
  //   url: "/",
  //   img: "../icons/logout.png"
  // },
  
  
];

export const fauxUsers = [
  { username: "user1", isCertified: true, subscriptionStatus: "Premium" },
  { username: "user2", isCertified: false, subscriptionStatus: "Free" },
  { username: "user3", isCertified: true, subscriptionStatus: "Premium" },
  { username: "user4", isCertified: false, subscriptionStatus: "Premium" },
  { username: "user5", isCertified: true, subscriptionStatus: "Free" },
  { username: "user6", isCertified: true, subscriptionStatus: "Premium" },
  { username: "user7", isCertified: false, subscriptionStatus: "Free" },
  { username: "user8", isCertified: true, subscriptionStatus: "Premium" },
  { username: "user9", isCertified: false, subscriptionStatus: "Premium" },
  { username: "user10", isCertified: true, subscriptionStatus: "Free" },
  { username: "user11", isCertified: true, subscriptionStatus: "Premium" },
  { username: "user12", isCertified: false, subscriptionStatus: "Free" },
  { username: "user13", isCertified: true, subscriptionStatus: "Premium" },
  { username: "user14", isCertified: false, subscriptionStatus: "Premium" },
  { username: "user15", isCertified: true, subscriptionStatus: "Free" },
  { username: "user16", isCertified: true, subscriptionStatus: "Premium" },
  { username: "user17", isCertified: false, subscriptionStatus: "Free" },
  { username: "user18", isCertified: true, subscriptionStatus: "Premium" },
  { username: "user19", isCertified: false, subscriptionStatus: "Premium" },
  { username: "user10", isCertified: true, subscriptionStatus: "Free" }
];

export const stories = [
  {
      
      name: "Adventure Time",
      img: "../background/bgimg.jpg"
  },
  {
          name: "Lost in Space",
      img: "../background/bgimg.jpg"
  },
  {
   
      name: "Mystery Island",
      img: "../background/bgimg.jpg"
  },
  {
     
      name: "The Forest",
      img: "../background/bgimg.jpg"
  },
  {
     
      name: "Underwater Quest",
      img: "../background/bgimg.jpg"
  },
  {
  
    name: "Adventure Time",
    img: "../background/bgimg.jpg"
},
{
 
    name: "Lost in Space",
    img: "../background/bgimg.jpg"
},
{
 
    name: "Mystery Island",
    img: "../background/bgimg.jpg"
},
{
   
    name: "The Forest",
    img: "../background/bgimg.jpg"
},
{
   
    name: "Underwater Quest",
    img: "../background/bgimg.jpg"
},
{
  
  name: "Adventure Time",
  img: "../background/bgimg.jpg"
},
{
      
  name: "Adventure Time",
  img: "../background/bgimg.jpg"
},
{
      name: "Lost in Space",
  img: "../background/bgimg.jpg"
},
{

  name: "Mystery Island",
  img: "../background/bgimg.jpg"
},
{
 
  name: "The Forest",
  img: "../background/bgimg.jpg"
},
{
 
  name: "Underwater Quest",
  img: "../background/bgimg.jpg"
},
{

name: "Adventure Time",
img: "../background/bgimg.jpg"
},
{

name: "Lost in Space",
img: "../background/bgimg.jpg"
},
{

name: "Mystery Island",
img: "../background/bgimg.jpg"
},
{

name: "The Forest",
img: "../background/bgimg.jpg"
},
{

name: "Underwater Quest",
img: "../background/bgimg.jpg"
},
{

name: "Adventure Time",
img: "../background/bgimg.jpg"
},
{
  name: "Lost in Space",
  img: "../background/bgimg.jpg"
},
{

  name: "Mystery Island",
  img: "../background/bgimg.jpg"
},
{

  name: "The Forest",
  img: "../background/bgimg.jpg"
},
{
 
  name: "Underwater Quest",
  img: "../background/bgimg.jpg"
}
];
export const liveUsers = [
  { id: 1, username: 'user1', avatar: '../profileImg.png' },
  { id: 2, username: 'user2', avatar: '../profileImg.png' },
  { id: 3, username: 'user3', avatar: '../profileImg.png' },
  { id: 4, username: 'user4', avatar: '../profileImg.png' },
  { id: 5, username: 'user5', avatar: '../profileImg.png' },
  // Add more users as needed
];
export const placeholderPosts = [
  {
    id: "1",
    user_id: "e0612c8e-2e87-4e64-bdb3-b99bbfbaa5fd",
    caption: "Embark on an unforgettable adventure with a visit to the spectacular Cascade Caverns.",
    post_link: "",
    post_link_title: "",
    post_link_image: "",
    attachment_file: "../images/cascade-caverns.jpg",
    attachment_file_name: "",
    attachment_type: "image",
    post_share: "",
    post_privacy: "",
    post_type: "",
    creation_date: "2024-07-27T12:00:00Z",
    comments_privacy: "",
    comments: [],
    reactions: [],
    likes: [],
    tips: []
  },
  {
    id: "2",
    user_id: "e0612c8e-2e87-4e64-bdb3-b99bbfbaa5fd",
    caption: "Dive into the world of culinary wonders and discover the best local foods that tantalize your taste buds.",
    post_link: "",
    post_link_title: "",
    post_link_image: "",
    attachment_file: "../images/local-eats.jpg",
    attachment_file_name: "",
    attachment_type: "image",
    post_share: "",
    post_privacy: "",
    post_type: "",
    creation_date: "2024-07-26T11:00:00Z",
    comments_privacy: "",
    comments: [],
    reactions: [],
    likes: [],
    tips: []
  },
  {
    id: "3",
    user_id: "e0612c8e-2e87-4e64-bdb3-b99bbfbaa5fd",
    caption: "Get close to nature and experience wildlife like never before on a thrilling safari adventure.",
    post_link: "",
    post_link_title: "",
    post_link_image: "",
    attachment_file: "../videos/city-landscapes.mp4",
    attachment_file_name: "",
    attachment_type: "video",
    post_share: "",
    post_privacy: "",
    post_type: "",
    creation_date: "2024-07-25T10:00:00Z",
    comments_privacy: "",
    comments: [],
    reactions: [],
    likes: [],
    tips: []
  },
  {
    id: "4",
    user_id: "e0612c8e-2e87-4e64-bdb3-b99bbfbaa5fd",
    caption: "Join us as we uncover the secrets and sights of the world's most fascinating urban landscapes.",
    post_link: "",
    post_link_title: "",
    post_link_image: "",
    attachment_file: "../images/safari-adventure.jpg",
    attachment_file_name: "",
    attachment_type: "image",
    post_share: "",
    post_privacy: "",
    post_type: "",
    creation_date: "2024-07-24T09:00:00Z",
    comments_privacy: "",
    comments: [],
    reactions: [],
    likes: [],
    tips: []
  },
  {
    id: "5",
    user_id: "e0612c8e-2e87-4e64-bdb3-b99bbfbaa5fd",
    caption: "Soak up the sun and enjoy the serene beauty of our handpicked tropical paradise beaches.",
    post_link: "",
    post_link_title: "",
    post_link_image: "",
    attachment_file: "../videos/mahogany.mp4",
    attachment_file_name: "",
    attachment_type: "video",
    post_share: "",
    post_privacy: "",
    post_type: "",
    creation_date: "2024-07-23T08:00:00Z",
    comments_privacy: "",
    comments: [],
    reactions: [],
    likes: [],
    tips: []
  },
  {
    id: "6",
    user_id: "e0612c8e-2e87-4e64-bdb3-b99bbfbaa5fd",
    caption: "Soak up the sun and enjoy the serene beauty of our handpicked tropical paradise beaches.",
    post_link: "",
    post_link_title: "",
    post_link_image: "",
    attachment_file: "../videos/Happy Penis.mp4",
    attachment_file_name: "",
    attachment_type: "video",
    post_share: "",
    post_privacy: "",
    post_type: "",
    creation_date: "2024-07-22T07:00:00Z",
    comments_privacy: "",
    comments: [],
    reactions: [],
    likes: [],
    tips: []
  }
];



export const creatorGallery =  [
 
  {
    
     width: 10,
     height:  10,
    src: "../images/safari-adventure.jpg",
  },
  {
    
    width: 10,
    height:  10,
    src: "../images/safari-adventure.jpg",
  },
  {
    
    width: 10,
    height:  10,
    src: "../images/safari-adventure.jpg",
  },
  {
  
    width: 10,
    height:  10,
    src: "../images/safari-adventure.jpg",
  },
  {
  
    width: 10,
    height:  10,
    src: "../images/safari-adventure.jpg",
  },
  {
  
    width: 10,
    height:  10,
    src: "../images/safari-adventure.jpg",
  },
  {
  
    width: 10,
    height:  10,
    src: "../images/safari-adventure.jpg",
  },
  {
    
    width: 10,
    height:  10,
    src: "../images/safari-adventure.jpg",
  },
  {
    
    width: 10,
    height:  10,
    src: "../images/safari-adventure.jpg",
  },
  {
   
     width: 10,
     height:  10,
    src: "../images/safari-adventure.jpg",
  },
  {
   
    width: 10,
    height:  10,
    src: "../images/safari-adventure.jpg",
  },
  {
    
     width: 10,
     height:  10,
    src: "../images/safari-adventure.jpg",
  }
]