...
function SettingsComponent({ setActiveUrl}) {
  ...
  const renderSettingsLinks = () => (
    settings
      ...
      .map((setting, id) => (
        <div
          key={id}
          className={`flex justify-between items-center py-1 text-lg font-semibold transition-all ${activeIndex === id ? 'bg-color-lightGrey border-r-4 border-r-color-pink' : ''}`}
  ...
  return (
    <>
    <div className=" md:mt-7 h-[100vh] md:h-[80%] flex flex-col w-full">
      ...
      <div className="pt-4 w-full h-full md:h-[75%] md:sticky md:rounded-xl messages-chat-list overflow-x-clip  px-4 md:px-0 bg-color-white">
        {/* change the icon */}
        ...
        {/* <div className="space-y-6">
          {settings.map((setting, id) => (
            <Link
              to={setting.url}
              key={id}
              className={`flex justify-between items-center py-1 text-lg font-semibold transition-all ${
                activeIndex === id
                  ? 'bg-color-lightGrey border-r-4 border-r-color-pink'
                  : ''
              }`}
              onClick={() => {
                setActiveIndex(id);
                setActiveUrl(setting.id);
              }}
            >
              <span className="md:text-[0.8rem] text-[1rem] pl-3">{setting.title}</span>
              <img
